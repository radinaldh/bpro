...
function export_to_excel()
{
    if (isset($_POST['export']) && $_POST['export'] === 'true') {
        $name_filter = sanitize_text_field($_POST['name']);
        $confirmed_filter = sanitize_text_field($_POST['confirmed']);
        ...
        if ($query->have_posts()) {
            ...
            header("Content-Disposition: attachment; filename=submissions_list.xls");

            echo "Name\tEmail\tPhone\tConfirmed\n";
            ...
            while ($query->have_posts()) : $query->the_post();
                ...
                $checked_in = get_post_meta(get_the_ID(), 'checked_in', true);
                $confirmed = $checked_in === 'true' ? 'Yes' : 'No';
...
add_action('admin_post_nopriv_export_to_excel', 'export_to_excel');
